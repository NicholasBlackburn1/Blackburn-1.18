...
public class WorldGenSettings
{
    ...
    public WorldGenSettings(long pSeed, boolean p_64610_, boolean pGenerateFeatures, MappedRegistry<LevelStem> pGenerateBonusChest)
    {
        this(pSeed, p_64610_, pGenerateFeatures, pGenerateBonusChest, Optional.empty());
    ...
    public static MappedRegistry<LevelStem> withOverworld(Registry<DimensionType> pDimensions, MappedRegistry<LevelStem> pDimensionTypes, ChunkGenerator pChunkGenerator)
    {
        LevelStem levelstem = pDimensionTypes.get(LevelStem.OVERWORLD);
        Supplier<DimensionType> supplier = () ->
        {
            return levelstem == null ? pDimensions.getOrThrow(DimensionType.OVERWORLD_LOCATION) : levelstem.type();
        };
        return withOverworld(pDimensionTypes, supplier, pChunkGenerator);
    }
