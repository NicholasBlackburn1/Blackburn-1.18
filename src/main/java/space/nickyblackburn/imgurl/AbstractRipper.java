package space.nickyblackburn.imgurl;
...
public abstract class AbstractRipper
    ...
    protected void writeDownloadedURL(String downloadedURL) throws IOException {
        ...
        try {
            ...
            if (!new File(Utils.getConfigDir()).exists()) {
                Consts.error("Config dir doesn't exist");
                Consts.info("Making config dir");
                boolean couldMakeDir = new File(Utils.getConfigDir()).mkdirs();
                if (!couldMakeDir) {
                    Consts.error("Couldn't make config dir");
                    return;
                }
            }
            ...
            if (!file.exists()) {
                ...
                if (!couldMakeDir) {
                    Consts.error("Couldn't url history file");
                    return;
                }
            }
            if (!file.canWrite()) {
                Consts.error("Can't write to url history file: " + URLHistoryFile);
                return;
            }
    ...
    protected boolean addURLToDownload(URL url, String prefix, String subdirectory, String referrer, Map<String, String> cookies, String fileName, String extension, Boolean getFileExtFromMIME) {
        ...
        if (url.toExternalForm().equals("http:") || url.toExternalForm().equals("https:")) {
            Consts.info(url.toExternalForm() + " is a invalid url amd will be changed");
            return false;
        ...
        if (url.toExternalForm().contains(" ")) {
            ...
            try {
                ...
                Consts.error("Unable to remove spaces from url\nURL: " + url.toExternalForm());
                e.printStackTrace();
            }
        }
        ...
        try {
            ...
            Consts.debug("Ripper has been stopped");
            return false;
        }
        Consts.debug("url: " + url + ", prefix: " + prefix + ", subdirectory" + subdirectory + ", referrer: " + referrer + ", cookies: " + cookies + ", fileName: " + fileName);
        ...
        try {
            ...
            Consts.error("[!] Error creating save file path for URL '" + url + "':", e);
            return false;
        }
        Consts.debug("Downloading " + url + " to " + saveFileAs);
        if (!saveFileAs.getParentFile().exists()) {
            Consts.info("[+] Creating directory: " + Utils.removeCWD(saveFileAs.getParent()));
            saveFileAs.getParentFile().mkdirs();
        }
        if (Utils.getConfigBoolean("remember.url_history", true) && !isThisATest()) {
            Consts.info("Writing " + url.toExternalForm() + " to file");
            try {
                writeDownloadedURL(url.toExternalForm() + "\n");
            } catch (IOException e) {
                Consts.debug("Unable to write URL history file");
            }
        }
    ...
    protected void waitForThreads() {
        Consts.debug("Waiting for threads to finish");
        completed = false;
    ...
    void checkIfComplete() {
        if (observer == null) {
            Consts.debug("observer is null");
            return;
        }

        if (!completed) {
            completed = true;
            Consts.info("   Rip completed!");
